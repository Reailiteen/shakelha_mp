...
class MultiplayerEnemyUi extends StatelessWidget {
  const MultiplayerEnemyUi({
    Key? key, 
    required this.name, 
        ...
        children: [
          ...
          Expanded(
            ...
            child: Builder(builder: (context) {
              // Get my player info from room
              final mySocketId = socketMethods.socketClient.id;
              final myPlayer = mySocketId == null ? null : room?.players.firstWhere(
                (p) => p.socketId == mySocketId,
                orElse: () => room!.players.first,
              );
              final myName = myPlayer?.nickname ?? '—';
              final myPoints = myPlayer?.score ?? 0;
              ...
              Widget buildHeaderCard({
                ...
                return LayoutBuilder(
                  builder: (context, c) {
                    ...
                    Widget textBlock(bool flipx) => Transform.flip(
                      flipX: !flipx, 
                      child: Stack(
                        children: [
                          Positioned(
                        ...
                        children: [
                          Container(
                            ...
                            child: Padding(
                              ...
                              child: Center(child: textBlock(avatarOnLeft))
                            ),
                          ),
                          Positioned(
                            top: 0,
