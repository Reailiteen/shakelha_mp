...
class MultiplayerBoardUI extends StatelessWidget {
  const MultiplayerBoardUI({Key? key, required this.socketMethods}) : super(key: key);
              ...
              child: Padding(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final existingTile = room?.board.getTileAt(pos);
                    final placedTile = roomDataProvider.getTileAtPosition(pos);
                    final hoverPreview = roomDataProvider.getHoverAtPosition(pos);
                    
                    final displayLetter = existingTile?.letter ?? 
                                        placedTile?.letter ?? 
                                        hoverPreview?.letter ?? '';
                    final displayPoints = existingTile?.value ?? 
                                        placedTile?.value ?? 
                                        1;
                    ...
                    return DragTarget<Object>(
                      onWillAccept: (data) {
                        ...
                        final isEmpty = (existingTile == null) && 
                                       (placedTile == null) &&
                                       !roomDataProvider.isTileAtPosition(pos);
                      ...
                      onAccept: (data) {
                        ...
                        if (data is Tile) {
                          // Place a new tile from rack
                          roomDataProvider.addPlacedTile(
                            PlacedTile(tile: data, position: pos, isNewWord: true)
                          );
                          
                          // Emit hover update for real-time preview
                          socketMethods.hoverTile(
                            room.id, 
                            letter: data.letter, 
                            row: row, 
                            col: col
                          ...
                          // Move existing placed tile
                          roomDataProvider.moveTile(data.position, pos);
                          
                          // Clear old hover and set new one
                          socketMethods.clearHover(room.id);
                          socketMethods.hoverTile(
                            room.id, 
                      ...
                      builder: (context, candidate, rejected) {
                            ...
                            if (displayLetter.isNotEmpty)
                              Center(
                                child: FractionallySizedBox(
                                  widthFactor: 0.9, 
                                  heightFactor: 0.9, 
                                  child: tileVisual(displayLetter, displayPoints, 20)
                                )
                              ),
                            if (isHovering)
                        ...
                        if (placedTile != null) {
                          return Draggable<PlacedTile>(
                            data: PlacedTile(tile: placedTile, position: pos, isNewWord: true),
                            feedback: Material(
                              color: Colors.transparent,
                              child: SizedBox(
                                width: 28, 
                                height: 28, 
                                child: tileVisual(displayLetter, displayPoints, 28)
                              ),
                            ),
                            ...
                            child: GestureDetector(
                              onDoubleTap: () {
                                roomDataProvider.removeTile(pos);
                                if (room != null) {
                                  socketMethods.clearHover(room.id);
                                }
                              },
